import { industrialDataRight } from '../data/industrialData.js';

// function to generate the industrial right side html dynamically
function generateIndustrialRightSide() {
	// Define an empty variable
	let industrialRight = '';

	// Loop through the industrialDataRight array
	industrialDataRight.forEach((data) => {
		industrialRight =
			industrialRight +
			` 
                    <ul
                             ...
                             <div class="text-center lg:pl-[15px] lg:flex-1 lg:h-full lg:flex lg:flex-col lg:justify-center">
                                 ...
                                 <p
								   class=" lg:text-left lg:mb-[10px] custom-color-p text-[14px] mb-[5px] hover:underline hover:text-[#2e3185]"
								    title="${data.industrialPurposes.item1}"  
								   >
								       <a href="#">
									   ${data.industrialPurposes.item1}
									   </a>
									 </p> 
                                 <p
								  class="lg:text-left lg:mb-[10px] custom-color-p text-[14px] mb-[5px] hover:underline hover:text-[#2e3192]"
								     title="${data.industrialPurposes.item2}" 
								  >
								    <a href="#">
								    ${data.industrialPurposes.item2}
									</a>
									</p> 
                                 <p
								   class="lg:text-left lg:mb-[10px] custom-color-p text-[14px] mb-[5px] hover:underline hover:text-[#2e3192]"
								     title="${data.industrialPurposes.item3}"
									 >
									    <a href="#">
										${data.industrialPurposes.item3}
										</a>
									 </p> 
									    <p
								          class="lg:text-left lg:mb-[10px] custom-color-p text-[14px] mb-[5px] hover:underline hover:text-[#2e3192]"
								     title="${data.industrialPurposes.item4}"
									 >
									    <a href="#">
										${data.industrialPurposes.item4}
										</a>
									 </p> 
                             </div>
                        </li>
	...
	// return industrialRight variable
	return industrialRight;
}

// function to render the Industrial right side
export function renderIndustrialRightSide() {
	// Render the right side
	const rightSideContents = generateIndustrialRightSide();
