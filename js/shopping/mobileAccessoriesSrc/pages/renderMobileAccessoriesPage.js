import {
	mobileAccessoriesDataFirst,
	mobileAccessoriesDataSecond,
	mobileAccessoriesDataThird,
	mobileAccessoriesDataFourth,
	mobileAccessoriesDataFifth,
} from '../data/mobileAccessoriesData.js';
import { generateMobileAccessoriesTopLeft } from '../mobileAccessoriesComponent/card/topLeftCard.js';
import { generateMobileAccessoriesBottomLeft } from '../mobileAccessoriesComponent/card/bottomLeftCard.js';
import { generateMobileAccessoriesCenterBottom } from '../mobileAccessoriesComponent/card/centerBottomCard.js';
import { generateLadiesWearCenterTop } from '../../ladiesSrc/ladiesComponents/card/CenterTopCard.js';
import { generateMobileAccessoriesRight } from '../mobileAccessoriesComponent/card/rightCard.js';
import { renderFrameView } from '../../shared/frameView.js';
import { renderFormView } from '../../shared/formView.js';
export function renderMobileAccessoriesHTML() {
	...
	topDiv.innerHTML = ` 
            <i class="cursor-pointer flex items-center justify-center rounded-[50%] mr-[15px] shadow-ladies-wear-box-shadow">
              <img
               src="/build/img/mobile-accessories.png"
                alt="Mobile Accessories"
				loading="lazy"
                 />
             </i>
             <h2 class="text-base font-medium lg:text-[27px] lg:font-bold">Mobile Accessories</h2>
    `;
	...
	// Loop through mobileAccessoriesDataFirst and generate the top left items
	mobileAccessoriesDataFirst.forEach((product) => {
		const productItems = generateMobileAccessoriesTopLeft(product);
	...
	// Loop through mobileAccessoriesDataSecond and generate the bottom left items
	mobileAccessoriesDataSecond.forEach((product) => {
		const productItems = generateMobileAccessoriesBottomLeft(product);
	...
	// Loop through mobileAccessoriesDataThird and generate the center bottom items
	mobileAccessoriesDataThird.forEach((product) => {
		const productItems = generateMobileAccessoriesCenterBottom(product);
	...
	// Loop through mobileAccessoriesDataFourth and generate the center bottom items
	mobileAccessoriesDataFourth.forEach((product) => {
		const productItems = generateLadiesWearCenterTop(product);
	...
	// Loop through mobileAccessoriesDataFifth and generate the right items
	mobileAccessoriesDataFifth.forEach((product) => {
		const productItems = generateMobileAccessoriesRight(product);
