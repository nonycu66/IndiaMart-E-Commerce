import { machineryDataRight } from '../data/machineryData.js';

// function to generate the machinery right side html dynamically
function generateMachineryRightSide() {
	// Define an empty variable
	let machineryRight = '';

	// Loop through the machineryDataRight array
	machineryDataRight.forEach((data) => {
		machineryRight =
			machineryRight +
			` 
               	 <ul
                             ...
                             <div class="text-center lg:pl-[15px] lg:flex-1 lg:h-full lg:flex lg:flex-col lg:justify-center">
                                 ...
                                 <p
								   class=" lg:text-left lg:mb-[10px] custom-color-p text-[14px] mb-[5px] hover:underline hover:text-[#2e3185]"
								    title="${data.machineryPurposes.item1}"  
								   >
								       <a href="#">
									   ${data.machineryPurposes.item1}
									   </a>
									 </p> 
                                 <p
								  class="lg:text-left lg:mb-[10px] custom-color-p text-[14px] mb-[5px] hover:underline hover:text-[#2e3192]"
								     title="${data.machineryPurposes.item2}" 
								  >
								    <a href="#">
								    ${data.machineryPurposes.item2}
									</a>
									</p> 
                                 <p
								   class="lg:text-left lg:mb-[10px] custom-color-p text-[14px] mb-[5px] hover:underline hover:text-[#2e3192]"
								     title="${data.machineryPurposes.item3}"
									 >
									    <a href="#">
										${data.machineryPurposes.item3}
										</a>
									 </p> 
								   <p
                                   class="lg:text-left lg:mb-[10px] custom-color-p text-[14px] mb-[5px] hover:underline hover:text-[#2e3192]"
								     title="${data.machineryPurposes.item4}"
									 >
									    <a href="#">
										${data.machineryPurposes.item4}
										</a>
									 </p> 
                             </div>
                        </li>
	...
	// Return machineryRight variable
	return machineryRight;
}

// Function to render the Machinery right side
export function renderMachineryRightSide() {
	// Render the right side
	const rightSideContents = generateMachineryRightSide();
