import { logisticsDataRight } from '../data/logisticsData.js';

// function to generate the logistics right side html dynamically
function generateLogisticsRightSide() {
	// Define an empty variable
	let logisticsRight = '';

	// Loop through the logisticsDataRight array
	logisticsDataRight.forEach((data) => {
		logisticsRight =
			logisticsRight +
			`
                  <ul
                             ...
                             <div class="text-center lg:pl-[15px] lg:flex-1 lg:flex lg:flex-col lg:justify-center md:h-full lg:h-full">
                                 ...
                                 <p
								   class=" lg:text-left lg:mb-[10px] custom-color-p text-[14px] mb-[5px] hover:underline hover:text-[#2e3185]"
								    title="${data.logisticsPurposes.item1}"  
								   >
								       <a href="#">
									   ${data.logisticsPurposes.item1}
									   </a>
									 </p> 
                                 <p
								  class="lg:text-left lg:mb-[10px] custom-color-p text-[14px] mb-[5px] hover:underline hover:text-[#2e3192]"
								     title="${data.logisticsPurposes.item2}" 
								  >
								    <a href="#">
								    ${data.logisticsPurposes.item2}
									</a>
									</p> 
                                 <p
								   class="lg:text-left lg:mb-[10px] custom-color-p text-[14px] mb-[5px] hover:underline hover:text-[#2e3192]"
								     title="${data.logisticsPurposes.item3}"
									 >
									    <a href="#">
										${data.logisticsPurposes.item3}
										</a>
									 </p> 
									   <p
								        class="lg:text-left lg:mb-[10px] custom-color-p text-[14px] mb-[5px] hover:underline hover:text-[#2e3192]"
								     title="${data.logisticsPurposes.item4}"
									 >
									    <a href="#">
										${data.logisticsPurposes.item4}
										</a>
									 </p> 
                                   </div>
                               </li>
	...
	return logisticsRight;
}

// Function to render the logistics right side
export function renderLogisticsRightSide() {
	// Render the right side
	const rightSideContents = generateLogisticsRightSide();
