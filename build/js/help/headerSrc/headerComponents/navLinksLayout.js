...
export function generateNavLinksLayout() {
	...
	headerData.forEach((data, index) => {
		// Determine if the current nav link is the third nav link
		const isThirdItem = index === 2;
		// Determine if the current nav link is the last nav link
		...
		// If the current nav link is the third nav link, generate the help dropdown html
		if (isThirdItem) {
            ...
            `;
			// If the current nav link is the last nav link, generate the signin dropdown html
		} else if (isLastItem) {
			...
			` 
			     ...
			     class="mt-0 ml-[5px] mr-[5px] py-0 px-2 custom-line-height relative list-none"
                  ...
                  <a
					...
					style="background-position: ${data.navLink.navIconPosition}">
					</span>
					${data.navLink.text}
				    </a>
                    ${helpDropdownHTML}
