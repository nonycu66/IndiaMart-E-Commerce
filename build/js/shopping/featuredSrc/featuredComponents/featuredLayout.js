//  Function to generate the featured products contents
export function generateFeaturedLayoutHTML(product, index, totalLength) {
	...
	card.innerHTML = `
                ...
                <div class=" py-[24px] px-[10px]">
                    <a href="/products/?id=${product.id}" 
                        class="block text-[#111]">
                    ...
                    <span>
                        <a href="/products/?id=${product.id}" class="flex items-center justify-center lg:justify-start">
                            ...
                            'Featured_Product_Homepage_PriceClicked',
                              '${product.store}', '');">
                                ${product.price}</p>
                            <del class=" text-base ml-[10px] letter-spacing-2 font-medium">${product.discountedPrice}</del>
                            <span class="ml10 fs16 fw6 dscnt text-base text-[#f7a813] font-semibold ml-[10px]">${product.discountedPercentage}</span>
                        </a>
                    </span>
                    <span class="font-bold text-sm mt-[10px] cursor-pointer flex items-center justify-center px-[10px] py-[10px] rounded-[4px] border border-solid border-[#2e3192] text-[#2e3192] hover:bg-[#2e3192] hover:text-white ">${product.buyButton}</span>
                </div>
            `;
	return card;
}
