import {
	ladiesWearDataFirst,
	ladiesWearDataSecond,
	ladiesWearDataThird,
	ladiesWearDataFourth,
	ladiesWearDataFifth,
} from '../data/ladiesWearData.js';

import { generateLadiesWearTopLeft } from '../ladiesComponents/card/TopLeftCard.js';
import { generateLadiesWearBottomLeft } from '../ladiesComponents/card/BottomLeftCard.js';
import { generateLadiesWearCenterBottom } from '../ladiesComponents/card/CenterBottomCard.js';
import { generateLadiesWearCenterTop } from '../ladiesComponents/card/CenterTopCard.js';
import { generateLadiesWearRight } from '../ladiesComponents/card/RightCard.js';
import { renderFrameView } from '../../shared/frameView.js';
import { renderFormView } from '../../shared/formView.js';

export function renderLadiesWearHTML() {
	...
	topDiv.innerHTML = ` 
            <i class="cursor-pointer flex items-center justify-center rounded-[50%] mr-[15px] shadow-ladies-wear-box-shadow">
              <img
               src="/build/img/ladies-wear.png"
                alt="Ladies Wear"
				loading="lazy"
                 />
             </i>
             <h2 class="text-base font-medium lg:text-[27px] lg:font-bold">Ladies Wear</h2>
    `;
	...
	// Loop through ladiesWearDataFirst and generate the top left items
	ladiesWearDataFirst.forEach((product) => {
		const productItems = generateLadiesWearTopLeft(product);
	...
	// Loop through ladiesWearDataSecond and generate the bottom left items
	ladiesWearDataSecond.forEach((product) => {
		const productItems = generateLadiesWearBottomLeft(product);
	...
	// Loop through ladiesWearDataThird and generate the center bottom items
	ladiesWearDataThird.forEach((product) => {
		const productItems = generateLadiesWearCenterBottom(product);
	...
	// Loop through ladiesWearDataFourth and generate the center bottom items
	ladiesWearDataFourth.forEach((product) => {
	...
	// Loop through ladiesWearDataFifth and generate the right items
	ladiesWearDataFifth.forEach((product) => {
		const productItems = generateLadiesWearRight(product);
